...
int main()
{
	...
	// -------------------------
	Shader geometryPassShader("shaders/geometryPass.vs", "shaders/geometryPass.fs");
	Shader ssaoShader("shaders/ssao.vs", "shaders/ssao.fs");
	Shader ssaoBlurShader("shaders/ssaoBlur.vs", "shaders/ssaoBlur.fs");
	Shader lightingPassShader("shaders/lightingPass.vs", "shaders/lightingPass.fs");
	Model mo("../resources/objects/nanosuit/nanosuit.obj");
	...
	glm::vec3 lightColor = glm::vec3(0.5, 0.5, 1.0);
	// render loop
	while (!glfwWindowShouldClose(window))
	{
		...
		geometryPassShader.setBool("inverseNormal", true);
		renderCube();
		geometryPassShader.setBool("inverseNormal", false);
		//nanosuit
		model = glm::mat4(1.0f);
		...
		glm::vec3 lightViewPos = glm::vec3(camera.GetViewMatrix() * glm::vec4(lightPos, 1.0));
		lightingPassShader.setVec3("light.Position", lightViewPos);
		lightingPassShader.setVec3("light.Color", lightColor);
		const float L = 0.09;
		const float Q = 0.032;
		lightingPassShader.setFloat("light.L", L);
		lightingPassShader.setFloat("light.Q", Q);
		renderQuad();
		//std::cout << glGetError() << " ";
